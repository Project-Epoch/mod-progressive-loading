...
void ItemLoader::OnBeforeItemTemplatesQueried(std::string& query)
{
    std::string table = "item_template";
...
void ItemLoader::OnBeforeLootTemplateQueried(std::string& query, std::string lootStoreName)
{
    query = Acore::StringFormat(
        "SELECT Entry, Item, Reference, Chance, QuestRequired, LootMode, GroupId, MinCount, MaxCount "
        "FROM %s WHERE (Item NOT IN (SELECT entry FROM forbidden_items WHERE (after_or_before = 0 && patch <= %u) || (after_or_before = 1 && patch >= %u)))",
        lootStoreName,
